...
* The top 15 rows in this case contain **`r round(sum(topFatalities$Fatalities) / sum(fatalities$Fatalities) * 100, 2)`%** of all the events, so this is valid.  

#### Injuries

```{r Injuries arranging}
injuries <- life %>%
    group_by(EVTYPE) %>%
...
Again `injuries` contains a lot of 0s (i.e. non-fatal events), I selected only the first 15 rows.  

* The top 15 rows in this case contain **`r round(sum(topInjuries$Injuries) / sum(injuries$Injuries) * 100, 2)`%** of all the events, so this is valid.  
...
So **excessive heat** and **tornados** take, respectively **`r round(fatalities_excessive_heat$ratio * 100,2)`%** and **`r round(fatalities_tornado$ratio * 100,2)`%** of the total amount of fatalities in the timespan 1995-2011.  
...
Tornados take **`r round(injuries_tornado$ratio * 100,2)`%** of the total injuries.
...
Considering just the top 10 results is acceptable, since they contain **`r round(sum(top_prop$Property_Damage)/sum(prop$Property_Damage)*100,2)`%** of the total amount of property damage from 1995 to 2011.  

#### Crop Damage

```{r Crop damage arranging}
crop <- economy %>%
    group_by(EVTYPE) %>%
...
The first 10 results contain **`r round(sum(top_crop$Crop_Damage)/sum(crop$Crop_Damage)*100,2)`%** of the total amount of crop damage from 1995 to 2011; therefore, it's acceptable to make this approximation.  
...
Floods are responsible for **`r round(prop_flood$ratio*100,2)`%** of the damage, while hurricanes and typhoons are accountable for **`r round(prop_ht$ratio*100,2)`%**; cumulatively, they are responsible for **`r round((prop_flood$ratio + prop_ht$ratio)*100,2)`%** of the total property damages in the years 1995-2011, which is more than half.

```{r Fractions (Crop Damage)}
crop_flood <- crop %>%
    filter(EVTYPE == 'FLOOD') %>%
    select('Crop_Damage') %>%
...
Floods have a small hand in crop damages too (about **`r round(crop_flood$ratio*100,2)`%** of the total); the most significant hand in crop damages is (as previously observed) due to **drought**, which is responsible for **`r round(crop_drought$ratio*100,2)`%** of the amount.  

## Results  

When it comes to human life:  

* Americans should pay particular attention to **tornados**, since they have been responsible for **`r round(injuries_tornado$ratio * 100,2)`%** of the injuries and **`r round(fatalities_tornado$ratio * 100,2)`%** of the deaths in America due to storm events;
* The other evident danger is **excessive heat**; it's accountable for **`r round(fatalities_excessive_heat$ratio * 100,2)`%** of fatalities, so families should invest in solutions in that sense.  

About the economic impact of natural disasters:  

* **Floods** are responsible for both great economic losses both to properties and to crops (**`r round(prop_flood$ratio*100,2)`%** and **`r round(crop_flood$ratio*100,2)`%** respectively);
* The biggest problem for crops turned out (more or less obviously) to be **drought**. A good **`r round(crop_drought$ratio*100,2)`%** of crop losses is due to drought, which is something to be addressed;
* Another small percentage of property damages is due to **hurricanes** and **typhoons**. Protection against these can prevent up to **`r round((prop_flood$ratio + prop_ht$ratio)*100,2)`%** of property damages.